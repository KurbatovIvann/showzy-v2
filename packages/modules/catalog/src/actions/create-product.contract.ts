 ...
 *   `companies.create`.
 * - `basePriceMinor` is a canonical non-negative money wire string; optional
...
export const createProductContract = defineActionContract({
  ...
  description:
    "Create a product in the staff member's active company, optionally with an initial set of variants, in one transaction. Takes a trimmed name, a non-negative base price in minor units, optional currency (default UAH), and optional variants (name plus an optional price override that must include its currency). Company id is never input. Re-submitting the identical payload with the same idempotency key returns the already-created product and does not insert duplicates.",
  principal: "staff",
  transport: "client",
