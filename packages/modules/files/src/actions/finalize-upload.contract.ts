 ...
 * Mechanical: `timeout: 15000` covers HEAD+GET+CopyObject of up to 10 MiB plus
...
export const finalizeUploadContract = defineActionContract({
  ...
  description:
    "Finalize a pending catalog upload in the active company. Reads the handshake staging object, then verifies size, magic bytes against the declared MIME, SHA-256 checksum, and the server-derived key prefix, then copies the bytes onto the durable catalog key. Executables, archives, and HEIC fail validation even when the declared MIME is an image. Missing or foreign-company files fail with not-found. A second finalize of a ready file returns the same view.",
  principal: "staff",
  transport: "client",
