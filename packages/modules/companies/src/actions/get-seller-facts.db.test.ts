...
import { getSellerFacts } from "./get-seller-facts.js";
  ...
  await kit.db.runtime.db.insert(user).values([
    {
      id: clerks.manager,
      name: "Manager",
      email: "manager@companies-seller-facts.test",
    },
    {
      id: clerks.employee,
      name: "Employee",
      email: "employee@companies-seller-facts.test",
    },
    {
      id: clerks.denied,
      name: "Denied employee",
      email: "denied@companies-seller-facts.test",
    },
  ]);
  await kit.db.runtime.db.insert(companyMembers).values([
    {
...
describe("companies.getSellerFacts", () => {
  it("returns identity with legal null when no legal row exists and prefix is present", async () => {
    const result = await kit.invoke(getSellerFacts, {});
  ...
  it("rejects any input identifier — the input is a strict empty object", async () => {
    await expect(
      kit.invoke(getSellerFacts, { companyId: kitIdentities.companies.a }),
    ).rejects.toBeInstanceOf(ValidationError);
    await expect(
      kit.invoke(getSellerFacts, { id: kitIdentities.companies.a }),
    ).rejects.toBeInstanceOf(ValidationError);
  });
