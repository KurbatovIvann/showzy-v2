/**
 * Order create draft, dirty detection, and line helpers (SHO-213).
...
export function isOrderFormDirty(
  draft: OrderFormDraft,
  origin: OrderFormDraft,
): boolean {
  if (
    draft.customerId !== origin.customerId ||
    draft.comment !== origin.comment
  ) {
    return true;
  }
  if (draft.items.length !== origin.items.length) {
    return true;
  }
  for (let index = 0; index < draft.items.length; index += 1) {
    const current = draft.items[index];
    const previous = origin.items[index];
    ...
    if (
      ...
      current.quantityMilli !== previous.quantityMilli
    ) {
      return true;
    }
  }
