...
| `form/`   | Create/edit screen, view, UI draft Zod, RHF `Controller` sections, save loop, unsaved guard, variant sheet. Pure roles: `product-form-draft.ts`, `product-form-plan.ts`, `product-form-copy.ts`, `product-form-load.ts` | List filters; photo session internals; a combined `*-model.ts`           |
