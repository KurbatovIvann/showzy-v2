/**
 * Dirty-leave for the document create form (SHO-238). Document-local:
...
export function useUnsavedDocumentGuard(args: {
  ...
  usePreventRemove(
    documentFormLeaveBlocked({ dirty, pending, leaveArmed }),
    ({ data }) => {
  ...
  useEffect(() => {
    ...
    const resolved = resolveArmedDocumentLeave(pendingLeaveActionRef.current);
