/**
 * Confirm / start / complete / cancel writes (SHO-378). Each write mints
 * a separate `useContractMutation` attempt so a failed confirm does not
 * retry as start. `requiresConfirmation` is false on the contracts.
 */
...
export function useOrderDetailActions(args: {
  ...
  const confirmMutation = useContractMutation(
    (input: OrderStatusWrite, options) =>
      bindOrderStatusMutate(apiRef.current)(input, options),
  );
