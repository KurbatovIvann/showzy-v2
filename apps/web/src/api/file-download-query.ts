 ...
 * Signed URLs are short-lived and live only in the in-memory query
 * cache — never in prefs or any persisted store. `staleTime` tracks
 * `expiresAt` so a re-observed row refetches a fresh URL instead of
