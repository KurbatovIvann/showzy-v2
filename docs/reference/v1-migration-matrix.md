...
| orders | `carts`, `cart_items`, `orders`, `order_items`, `order_logs`, `company_statuses` → TRANSFORM to fixed workflow (`orders`/`order_items` column mapping: orders spec §7); `user_checkout_preferences` → TRANSFORM in phase-4 checkout design (orders spec §7.6) | `status_templates`, `status_template_items`, `status_template_transitions`, `status_transitions`, `status_automations`, `status_template_auto_transitions`, `status_auto_transitions` → DROP; `checkout_sessions` → DROP (orders spec §7.6, owner 2026-08-17) |
