...
### Current (`main`, as of SHO-324)

Do **not** restructure this tree in a documentation ticket. Today
routes are mostly **dotted siblings** under `$companySlug/`, and
section files remount `SectionWorkspacePage` instead of nesting
`<Outlet />`. Panel chrome lives in `src/features/panel`. Composition
files live at `src/main.tsx`, `src/router.tsx`, `src/app-providers.tsx`.
Section / list-vs-detail / full-shell still use pathname helpers in
`features/panel/section-path.ts` — that is the gap later SHO-325
tickets close.
...
apps/web/src/routes/
  ...
  _authed/index.tsx                 # /
  _authed/onboarding.tsx
  _authed/$companySlug.tsx          # company layout (file) + Outlet
  _authed/$companySlug/
    index.tsx
    orders.tsx                      # sibling, not a nested outlet parent
    orders.$orderId.tsx
    orders.new.tsx
    documents.tsx
    ...
    documents.templates.$templateId.tsx
    ...
    customers.new.tsx
    customers.groups.tsx
    customers.groups.$groupId.tsx
    customers.groups.new.tsx
    customers.counterparties.tsx
    customers.counterparties.$counterpartyId.tsx
    customers.counterparties.new.tsx
    invites.tsx
    invites.$inviteId.tsx
    invites.new.tsx
    pricing.tsx
    pricing.$priceListId.tsx
    pricing.$priceListId.edit.tsx
    pricing.new.tsx
    company.tsx
    company.legal.tsx
    company.team.tsx
