...
| **Backup failure** | pgBackRest/cron non-zero; missing WAL for > RPO. A scheduled `--restore-smoke` job is planned at launch (CI today only runs `--dry-run`) | SEV2 | Host scheduler around `docs/operations/backups.md` |
