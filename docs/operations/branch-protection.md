...
2. **Require status checks to pass** with **"Require branches to be up to
   ...
   - `checks` — fail-closed aggregator over format, typecheck, lint,
     test-unit, test-db, build-smoke, and the other required jobs below
     (SHO-334, SHO-336). The job name is the stable branch-protection
     check; workers report independently.
